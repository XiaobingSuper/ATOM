"""Patch vLLM graph capture to also enter aiter capture contexts.
...
ca_comm.capture() and FlyDSL HGEMM+AR capture context, so both regular custom
collectives and fused GEMM+allreduce can use their graph-registered buffer
paths during vLLM plugin mode capture.
"""
...
def _get_flydsl_hgemm_ar_capture_context():
    """Lazily get FlyDSL HGEMM+AR capture context, or nullcontext if unavailable."""
    try:
    ...
    try:
        return _get_tp_flydsl_hgemm_ar_capture_context()
    except Exception:
        return nullcontext()


def _patched_graph_capture(original_graph_capture):
    """Wrap vLLM's graph_capture to also enter aiter capture contexts."""
    ...
    def wrapped(self, graph_capture_context=None):
        ...
        flydsl_hgemm_ar_context = _get_flydsl_hgemm_ar_capture_context()
        with aiter_ca_context, flydsl_hgemm_ar_context:
...
def apply_graph_capture_patch() -> None:
    """Patch vLLM's GroupCoordinator.graph_capture to nest aiter capture contexts."""
    ...
    try:
        ...
        logger.info(
            ...
            "and FlyDSL HGEMM+AR capture context"
        )
        ...
        logger.warning(
            ...
            "aiter collectives or FlyDSL HGEMM+AR may miss graph-registered "
            "buffer paths in vLLM plugin mode.",
            e,
        )
